...
expandBasin :: Grid -> [(Int, Int)] -> (Int, Int) -> [(Int, Int)]
expandBasin g traversed p =
  let candidates :: [(Int, Int)] = expandPoint g p
      forbidden :: [(Int, Int)] = filter (\(x, y) -> (g ! x ! y) == 9) candidates
      selected :: [(Int, Int)] = L.nub $ candidates L.\\ (forbidden ++ traversed)
      base = selected
      rec :: [(Int, Int)] =
        L.concatMap
          ( \x ->
              let traversed' = traversed ++ (base L.\\ [x])
               in expandBasin g traversed' x
          )
          base
   in if (L.length base > 1) then (base ++ rec) else base

findBasins :: Grid -> [(Int, Int)] -> [[(Int, Int)]]
findBasins g ps =
  let state = [(-9, -9)]
   in fmap (\p -> trace (show p) $ (L.nub . (expandBasin g state)) p) ps

expandPoint :: Grid -> (Int, Int) -> [(Int, Int)]
expandPoint g (x, y) =
  let fallback = (x, y)
